...
export default function ReserveStalls() {
  ...
  const [showReservationConfirmModal, setShowReservationConfirmModal] =
    useState(false);
    ...
    async function loadMyReservedStalls() {
      ...
      try {
        ...
        reservations.forEach((reservation) => {
          ...
          if (
            reservationEventId &&
            reservationEventId !== Number(activeEventId)
          ) {
            return;
          }
        ...
        setReservedLoadError(
          "Unable to load your existing reserved stalls right now.",
        );
      }
    }
  ...
  async function onConfirmReservationYes() {
    try {
      setSaving(true);
      ...
      sessionStorage.setItem("showGenrePromptAfterReservation", "1");
      if (newId) {
        sessionStorage.setItem("postReservationId", String(newId));
      } else {
          ...
          {activeEventInfo ? (
            <>
              ...
              <p className="mt-1 text-sm text-gray-600">
                {formatDate(activeEventInfo.startDate)} to{" "}
                {formatDate(activeEventInfo.endDate)}
              </p>
            </>
          ) : (
            <p className="mt-1 text-sm text-gray-600">
      ...
      <div className="grid grid-cols-1 gap-6 mt-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-1">
          <div className="p-4 bg-white border rounded-2xl">
            ...
            <p className="mt-1 text-sm text-gray-600">
              Already Reserved:{" "}
              <span className="font-semibold">{alreadyReservedCount}</span> /{" "}
              {maxAllowedStalls}
            </p>
            {reservedLoadError && (
              <p className="mt-1 text-xs text-amber-700">{reservedLoadError}</p>
            )}
            ...
            <button
              onClick={onConfirm}
              disabled={
                saving || selected.length === 0 || remainingSelectable <= 0
              }
            ...
            <p className="mt-3 text-xs text-gray-500">
              Reserved stalls (grey) cannot be selected. You can reserve up to 3
              stalls in total.
            </p>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="w-full max-w-lg p-6 bg-white shadow-xl rounded-2xl">
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button
                type="button"
                onClick={() => setShowReservationConfirmModal(false)}
                disabled={saving}
              ...
              <button
                type="button"
                onClick={onConfirmReservationYes}
                disabled={saving}
